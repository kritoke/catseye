...
let extract_file (config : t) (src : source_file) : Security_node.t list option =
  ...
  let use_bridge = config.ast_bridge || match src.lang with "javascript" | "typescript" | "svelte" | "ocaml" | "elixir" | "fsharp" | "nim" -> true | _ -> false in
...
let print_banner (config : t) (cr_count : int) (gleam_count : int) (js_count : int) (ts_count : int) (svelte_count : int) (ocaml_count : int) (ex_count : int) (fs_count : int) (nim_count : int) (dep_count : int) =
  Format.printf "
  %sCatseye v%s%s
...
" (styled green config config.target_dir);
  ...
  let lang_parts = if nim_count > 0 then (Printf.sprintf "%d Nim" nim_count) :: lang_parts else lang_parts in
  ...
  `Assoc [
    ("manifests", `List (List.map ~f:(fun m ->
      let (path, kind) = match m with
        ...
        | Catseye_crowsnest.Manifest.Nimble (p, _) -> (p, "nimble")
        | Catseye_crowsnest.Manifest.Nimble_lock (p, _) -> (p, "nimble.lock")
      in
...
let run (config : t) : int =
  ...
  let nim_count = List.length (List.filter ~f:(fun s -> s.lang = "nim") sources) in
  let dep_count = List.length (List.filter ~f:(fun s -> s.is_dependency) sources) in
  if config.format = Terminal then print_banner config cr_count gleam_count js_count ts_count svelte_count ocaml_count ex_count fs_count nim_count dep_count;
      ...
      (try
        ...
        | Ok mod_ ->
          (match mod_.mod_lang with
           ...
           | Nim ->
            List.map ~f:(convert_ai_finding ~lang:"nim") (Ai_linter.Nim_rules.analyze_module mod_)
           | _ -> [])
  ...
  (* Step 4f: nimalyzer integration — optional Nim analysis *)
  let all_findings =
    let nim_findings = Nimalyzer.run_if_configured
      ~enabled:config.nimalyzer_enabled
      ~config_path:config.nimalyzer_config_path
      ~is_terminal:(config.format = Terminal)
      ~language:"nim"
    in
    if config.format = Terminal && nim_findings <> [] then begin
      List.iter ~f:(print_finding config) nim_findings;
      Format.printf "\n  Found %d nimalyzer findings\n\n" (List.length nim_findings)
    end;
    all_findings @ nim_findings in
