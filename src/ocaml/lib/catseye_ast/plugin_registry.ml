...
type registry = {
  by_name : (string, Lp.t) Hashtbl.t;
  by_ext : (string, Lp.t) Hashtbl.t;
}

(** Create an empty registry. *)
let create () : registry = {
  by_name = Hashtbl.create 8;
  by_ext = Hashtbl.create 16;
}

(** Register a plugin. Raises Invalid_argument on duplicate name or extension. *)
let register (r : registry) (plugin : Lp.t) : unit =
  if Hashtbl.mem r.by_name plugin.Lp.name then
    invalid_arg (Printf.sprintf "Plugin '%s' already registered" plugin.Lp.name);
  List.iter (fun ext ->
    if Hashtbl.mem r.by_ext ext then
      invalid_arg (Printf.sprintf "Extension '%s' already claimed by another plugin" ext);
    Hashtbl.add r.by_ext ext plugin
  ) plugin.Lp.extensions;
  Hashtbl.add r.by_name plugin.Lp.name plugin

(** Look up a plugin by file extension (e.g. ".cr"). *)
let for_extension (r : registry) (ext : string) : Lp.t option =
  Hashtbl.find_opt r.by_ext ext

(** Look up a plugin by name (e.g. "crystal"). *)
let for_name (r : registry) (name : string) : Lp.t option =
  Hashtbl.find_opt r.by_name name

(** List all registered language names. *)
let languages (r : registry) : string list =
  Hashtbl.fold (fun name _ acc -> name :: acc) r.by_name []

(** Check if a language is available. *)
let available (r : registry) (name : string) : bool =
  Hashtbl.mem r.by_name name

(** All extensions across all plugins. *)
let all_extensions (r : registry) : string list =
  Hashtbl.fold (fun _ plugin acc ->
...
let lang_of_path (r : registry) (path : string) : string option =
  let rec try_exts = function
    | [] -> None
    | ext :: rest ->
      if Filename.check_suffix path ext then begin
