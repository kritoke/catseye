...
let extract_class_name (type_ref : string) : string =
  match String.index_opt type_ref '<' with
  | Some idx -> String.sub type_ref 0 idx
  | None -> 
    (match String.index_opt type_ref '|' with
     | Some idx -> String.sub type_ref 0 idx
     | None -> type_ref)
...
let normalize_class_name (name : string) : string =
  ...
  else if List.exists (fun p -> trimmed = p) ["String"; "Int32"; "Int64"; "Float"; "Bool"; "Number"; "Array"; "Hash"; "Symbol"; "Object"]
  then ""
  else trimmed
...
let count_unique_dependencies (nodes : Security_node.t list) 
  ...
  let add_dep name =
    if contains name "::" then
      ...
      if normalized <> "" then Hashtbl.replace deps normalized true
  in
  ...
  List.iter (fun node ->
    if node.Security_node.file = class_file then
      if node.Security_node.node_type = Security_node.Call then
        ...
        if contains name "." then
          (try
            let dot_idx = String.index name '.' in
            let receiver = String.sub name 0 dot_idx in
            add_dep receiver
          with Not_found -> ())
        else
          add_dep name
        ...
        if dep_count >= default_hub_threshold_critical then
          Some {
            ...
            flow = [ {
              Finding.file;
              line;
              message = Printf.sprintf "Definition of '%s' (%d class dependencies)"
                class_name dep_count;
            } ];
