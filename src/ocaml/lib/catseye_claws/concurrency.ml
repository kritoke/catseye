...
module OldList = struct
  ...
  let find_opt = Stdlib.List.find_opt
...
let extract_channel_actions (body : Security_node.t list) : channel_action list =
  ...
  let channel_vars : (string, bool) Stdlib.Hashtbl.t = Stdlib.Hashtbl.create 8 in
  OldList.iter (fun (n : Security_node.t) ->
    if n.Security_node.node_type = Security_node.Call then begin
      ...
      let nl = Stdlib.String.length name in
      if nl >= 4 && Stdlib.String.sub name (nl - 4) 4 = ".new" then begin
        (* Check if this is a Channel constructor *)
        let is_channel =
          let rec check i =
            i + 7 <= nl &&
            (Stdlib.String.sub name i 7 = "Channel" || check (i + 1))
          in check 0
        in
        if is_channel then begin
          (* Find the variable it was assigned to *)
          let assign = OldList.find_opt (fun (a : Security_node.t) ->
          ...
          (match assign with
           | Some a -> Stdlib.Hashtbl.replace channel_vars a.Security_node.name true
           | None ->
             (* Try next-line assign (e.g., ch = ... on same line as call) *)
             let assign2 = OldList.find_opt (fun (a : Security_node.t) ->
               a.Security_node.node_type = Security_node.Assign
               && a.Security_node.line >= n.Security_node.line
  ...
  OldList.filter_map (fun (n : Security_node.t) ->
    if n.Security_node.node_type <> Security_node.Call then None
    else
      let name = n.Security_node.name in
      let nl = Stdlib.String.length name in
      (* Channel send: var.send — only if var is a known channel *)
      if nl >= 5 && Stdlib.String.sub name (nl - 5) 5 = ".send" then begin
        let obj = Stdlib.String.sub name 0 (nl - 5) in
        if Stdlib.Hashtbl.mem channel_vars obj then Some (Send obj)
        else None
      end
      (* Channel receive: var.receive or var.receive? — 8 or 9 chars *)
      else if (nl >= 9 && Stdlib.String.sub name (nl - 9) 9 = ".receive?")
              || (nl >= 8 && Stdlib.String.sub name (nl - 8) 8 = ".receive")
      then begin
        let suffix_len = if nl >= 9 && Stdlib.String.sub name (nl - 9) 9 = ".receive?" then 9 else 8 in
        let obj = Stdlib.String.sub name 0 (nl - suffix_len) in
        if Stdlib.Hashtbl.mem channel_vars obj then Some (Receive obj)
        else None
      end
      (* Channel close: var.close *)
      else if nl >= 6 && Stdlib.String.sub name (nl - 6) 6 = ".close" then begin
        let obj = Stdlib.String.sub name 0 (nl - 6) in
        if Stdlib.Hashtbl.mem channel_vars obj then Some (Close obj)
        else None
      end
      else None
  ) body
...
let check_orphaned_spawn (body : Security_node.t list) : bool =
  let has_spawn = OldList.exists (fun (n : Security_node.t) ->
    n.Security_node.node_type = Security_node.Call
    && n.Security_node.name = "spawn"
  ) body in
  let has_rescue = OldList.exists (fun (n : Security_node.t) ->
  ...
  let has_ensure = OldList.exists (fun (n : Security_node.t) ->
    n.Security_node.node_type = Security_node.Call
    && (let name = n.Security_node.name in
        Stdlib.String.length name >= 7 &&
        Stdlib.String.sub name (Stdlib.String.length name - 7) 7 = "ensure")
  ) body in
  ...
  OldList.iter (fun (n : Security_node.t) ->
    if n.Security_node.node_type = Security_node.Def then begin
      (match !current_def with
       | Some d ->
         scopes := { def = d; body = OldList.rev !current_body } :: !scopes
       | None -> ());
      current_def := Some n;
      current_body := [];
...
let analyze (nodes : Security_node.t list) (_config : Types.claws_config)
  ...
  OldList.concat_map (fun ({ def; body } : scope) ->
    ...
    let muted_findings = OldList.map (fun (var, desc) ->
      { Finding.rule = "MutedPack";
        ...
        flow = [ {
          ...
          line = def.Security_node.line;
          message = Stdlib.Printf.sprintf "In '%s'" def.Security_node.name;
        } ];
        language = "crystal";
        dependency = None;
