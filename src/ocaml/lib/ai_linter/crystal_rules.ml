...
let String.sub = String.sub
let String.length = String.length
let String.index_opt = String.index_opt
let String.contains = String.contains
let List.concat_map = List.concat_map
...
let List.fold_left = List.fold_left

(* List.sort_uniq - sort and remove duplicates *)
let List.sort_uniq cmp l =
...
let List.concat_map = List.concat_map in
let List.iter = List.iter in
...
let List.concat_map = List.concat_map in
let String.sub = String.sub in
let String.length = String.length in
...
let List.concat_map = List.concat_map in
let std_length = List.length in
let std_filter = List.filter in
List.concat_map (fun item ->
match item.item_value with
...
let List.exists = List.exists in
let List.concat_map = List.concat_map in
...
let List.exists = List.exists in
let List.length = List.length in
let List.iter = List.iter in
let List.concat_map = List.concat_map in
...
| Crystal -> []
| _ ->
let List.filter = List.filter in
let List.length = List.length in
let List.iter = List.iter in
...
let List.filter_map = List.filter_map in
let List.concat_map = List.concat_map in
...
in
let List.iter = List.iter in
let List.fold_left = List.fold_left in
let rec count_nodes (e : expr) : int =
match e.expr_value with
| EBlock es -> List.fold_left (fun acc e -> acc + count_nodes e) 0 es
...
let List.exists = List.exists in
let List.find_opt = List.find_opt in
let List.concat_map = List.concat_map in
...
let min_co_occurrence = 3 in
let List.filter_map = List.filter_map in
let List.sort_uniq = List.sort_uniq in
let List.iter = List.iter in
let List.concat_map = List.concat_map in
let List.length = List.length in
let List.hd = List.hd in
let param_sets = List.filter_map (fun item ->
match item.item_value with
| IFunction (_, params, _, _) ->
Some (List.filter_map (function PVar v -> Some v | _ -> None) params)
...
let List.exists = List.exists in
let List.map = List.map in
let List.length = List.length in
let String.contains = String.contains in
...
| EApp (fn, _) ->
let name = get_full_name fn in
name = "not_nil!" || name = "try" || String.is_suffix name ".try"
| EIf (_, then_, else_) ->
