...
let group_by_file (nodes : Security_node.t list)
  ...
  let map = List.fold_left (fun m n ->
  ...
  |> List.map (fun (file, ns) -> (file, List.sort (fun a b -> compare a.Security_node.line b.Security_node.line) ns))
...
let track_return_taint (nodes : Security_node.t list) (db : Db.t) : Db.t =
  ...
  let file_map = List.fold_left (fun m (f, ns) -> StringMap.add f ns m)
    StringMap.empty file_groups in
