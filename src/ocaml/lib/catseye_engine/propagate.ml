...
let cleanse_sanitized_assigns (nodes : Security_node.t list) (db : Db.t) : Db.t =
  List.fold_left (fun acc node ->
    if node.Security_node.node_type <> Security_node.Assign then acc
    else
      ...
      let has_sanitizer_call =
        ...
        match Hashtbl.find_opt
          (List.fold_left (fun m n ->
            if n.Security_node.node_type = Security_node.Call then
              let k = n.Security_node.file ^ ":" ^ string_of_int n.Security_node.line in
              Hashtbl.replace m k n; m
            else m
          ) (Hashtbl.create 128) nodes) key with
...
let propagate (nodes : Security_node.t list) (db : Db.t) : Db.t =
  let rec loop db count =
    if count >= 100 then db
      ...
      let db' = cleanse_sanitized_assigns nodes (do_propagate nodes db) in
