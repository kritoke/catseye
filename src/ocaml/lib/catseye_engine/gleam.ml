...
let extract_nodes root path =
  ...
  let defs = List.filter_map (fun fn ->
    ...
    | Some name ->
      ...
      Some { node_type = Def; name; args = params
           ; line = line_of fn; taint = false; file = path }
  ...
  let assigns = List.concat_map (fun tag ->
    List.filter_map (fun (lt : xml) ->
      ...
      | tgt :: _ ->
        ...
        Some { node_type = Assign; name; args; line = line_of lt; taint; file = path }
    ) (find root ~tag)
  ) ["let"; "let_assert"] in
  ...
  let calls = List.filter_map (fun call ->
    ...
    | Some name ->
      ...
      Some { node_type = Call; name; args; line = line_of call; taint; file = path }
